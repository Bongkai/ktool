 ...
 *                              否则原数组不改变, 默认为 false
 * @param {Number} [fromIndex]  检索的起始位置下标, 默认为 arr.length - 1
 * @returns {Array}  返回删除后的数组
 * 
 * @example 
 * 详见 remove() 的例子
 */
function removeLast(arr, conditions, isOrigin, fromIndex) {
  return _removeItems(arr, conditions, isOrigin, fromIndex, 'last');
}
